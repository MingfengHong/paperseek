...
`TARGET_MAX` / `--max` 是最终输出上限，不要求数据源只返回这么多记录。检索阶段另有 `SEARCH_ACCEPT_MAX_RECORDS` / `--accept-max`，默认 `1000`。只要数据源总命中数不超过这个候选上限，PaperSeek 就可以进入后续候选池评分，再从中输出最相关的前 `TARGET_MAX` 条。
...
如果最后一轮仍然过少或超过候选上限，PaperSeek 会读取前几轮的查询、动作和命中数，让 LLM 额外重建一次兜底查询。兜底查询只执行一次，避免在网络或上游服务异常时无限循环。

### Candidate pool

候选池由数据源返回的论文构成。如果启用 OpenAlex 引用扩展，候选池还会加入高匹配论文的引用邻居。最终排序面向候选池，而不是只展示某一次数据源返回的原始顺序。

因此，`TARGET_MAX=50` 表示最终最多输出 50 条，不表示 OpenAlex 必须只命中 50 条以内。默认情况下，OpenAlex 命中 5 到 1000 条都可以进入排序阶段。PaperSeek 会分页拉取候选记录，并在候选较多时分批调用 LLM 排序；`Source Cap` 越高，请求次数、运行时间和模型成本越高。
...
引用扩展不是只抓一跳。PaperSeek 会把每一层新抓到的参考文献和被引论文先交给 LLM 评分；只有出现达到相关性阈值的高价值论文，才继续从这些论文向下一层扩展。停止条件包括：
...
引用扩展有助于发现关键词检索遗漏的经典文献、相邻主题和近期延伸研究，但会增加 OpenAlex 与 LLM 请求次数。
...
| `SEARCH_ACCEPT_MAX_RECORDS` | `1000` | 检索阶段可接受的源记录上限；最终输出仍由 `TARGET_MAX` 控制。 |
...
| `CITATION_MAX_DEPTH` | `3` | 引用扩展最大层数。 |
| `CITATION_RELEVANCE_THRESHOLD` | `7.0` | 继续向下一层扩展所需的最低相关性评分。 |
| `PAPERSEEK_HISTORY_ENABLED` | `true` | 是否启用本地 SQLite 历史记录。 |
| `PAPERSEEK_TIMEZONE` | 自动检测 | 后端历史时间的时区覆盖值，例如 `Asia/Shanghai`；检测失败默认东八区。 |
...
| `--initial-query` | 从已有检索式继续运行，跳过 LLM 查询生成。 |
| `--min` | 目标最少结果数。 |
| `--max` | 目标最多结果数，当前 UI 和导出上限为 50。 |
| `--accept-max` | 检索阶段可接受的源记录上限，默认 1000。 |
| `--iterations` | 最大查询调整轮数。 |
| `--citation-depth` | OpenAlex 引用扩展最大层数。 |
| `--citation-threshold` | 继续引用扩展所需的最低相关性分数。 |
...
如果一次运行在数据源请求、排序或引用扩展阶段失败，可以先查看历史记录中的 `final_query` 或日志里最后一次可用查询，然后从这个查询继续：

```bash
paperseek history show <RUN_ID> --json
paperseek search "原研究问题" --source openalex --initial-query "保存下来的检索式"
```

`--initial-query` 会跳过 Query Generation，直接进入 Source Search。这能减少网络失败后的重复 LLM 调用，也能保留人工修正后的检索式。

当前版本尚未把完整候选池、引用扩展中间层和排序输入持久化为可任意子阶段重放的缓存。因此它不是“从任意内部步骤恢复”的完整 checkpoint 系统；更细粒度的候选池和排序断点适合后续单独实现。
...
| 数据源 | 显示字段 |
| --- | --- |
| OpenAlex | OpenAlex API Key、OpenAlex Email、Field Hint、Expand citations、Citation Depth、Citation Threshold |
...
- `Min Results` 默认 `5`。
- `Max Results` 默认 `50`。
- `Source Cap` 默认 `1000`，表示数据源总命中数不超过该值时可以进入排序。
- `Iterations` 默认 `5`。
- `Resume Query` 仅在要从已有检索式继续时填写，平时留空。
...
- 导出文件名使用 `研究问题主题-日期时间-数据源-papers.csv`，日期时间优先采用浏览器时区，检测失败时使用东八区。
...
| `CITATION_MAX_DEPTH` / `--citation-depth` | `3` | 最多向前向后扩展多少层。 |
| `CITATION_RELEVANCE_THRESHOLD` / `--citation-threshold` | `7.0` | 新一层论文达到该分数才继续扩展。 |

引用扩展每一层都会经历“抓取邻居 -> LLM 评分 -> 只保留高价值相关论文作为下一层 frontier”。如果某一层没有达到阈值的论文，扩展会停止。
