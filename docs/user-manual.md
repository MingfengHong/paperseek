...
因此，`TARGET_MAX=50` 表示最终最多输出 50 条，不表示 OpenAlex 必须只命中 50 条以内。默认情况下，OpenAlex 命中 5 到 1000 条都可以进入排序阶段。PaperSeek 会分页拉取候选记录，再把候选池交给 LLM 一次性统一评分；`Source Cap` 越高，请求次数、提示词长度、运行时间和模型成本越高。
