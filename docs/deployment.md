...
1. 登录 ModelScope，点击 [Deploy to ModelScope](https://modelscope.cn/studios/fork?target=FayeRR/paperseek) fork 社区版 PaperSeek 创空间，并按页面提示创建自己的副本。
...
1. Sign in to ModelScope and click [Deploy to ModelScope](https://modelscope.cn/studios/fork?target=FayeRR/paperseek) to fork the community PaperSeek Studio.
