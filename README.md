...
如果启用 OpenAlex 引用扩展，PaperSeek 会从高匹配论文中选择 seed paper，沿参考文献和被引论文多层扩展；每层都会先判断是否仍有高价值相关论文，没有时停止。默认最大输出 50 条，检索阶段默认可接受最多 1000 条源记录作为后续排序候选；候选较多时会分页拉取，再交给 LLM 统一评分。
